if __name__ == '__main__':
    ...
    for seed in range(1):
        ...
        seed_everything(seed | (local_rank << 16))
        ...
        working_dir = numerated_folder(os.path.join(constants.MODELS_PATH, 'asl'), create=local_rank == 0)
        session_name = os.path.basename(working_dir)
        module = Module(
            working_dir, constants.EPOCHS, constants.STEPS_PER_EPOCH, constants.LR, data.n_features,
            constants.N_FEATURES,
            data.n_outputs, constants.MAX_LEN, constants.DROPOUT,
            constants.N_LAYERS, constants.N_HEADS, constants.MLP_RATIO, constants.ARCHITECTURE,
            constants.CAUSAL_FORESIGHT,
            constants.EMA, constants.LABEL_SMOOTHING, constants.MIXUP, constants.MIXUP_ELEMENTS, constants.N_COORDS
        )
        ...
        trainer = pl.Trainer(
            ...
            strategy=('ddp_find_unused_parameters_true' if constants.CAUSAL_FRACTION
                      else 'ddp_find_unused_parameters_false')
            if isinstance(constants.DEVICES, list) or constants.DEVICES > 1 else 'auto',
            precision='16-mixed' if constants.ARCHITECTURE == 'lstm' else 'bf16-mixed',
